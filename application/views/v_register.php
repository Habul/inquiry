...
<body>
	<div class="wrapper">
		...
		<form class="p-3 mt-3" action="<?php echo base_url() . 'login/register_proses' ?>" onsubmit="regbtn.disabled = true; return true;" id="loginform" method="post">
			<div class="form-field d-flex align-items-center"><span class="fas fa-user-tie"></span>
				<input type="text" name="nama" placeholder="Full Name.." value="<?= set_value('name'); ?>" autofocus required>
			</div>
			...
			<div class="form-field d-flex align-items-center"><span toggle="#password-field2" class="fas fa-fw fa-lock field-icon toggle-password2"></span>
				<input type="password" name="password2" id="password-field2" placeholder="Retype Password.." required>
			</div>
			<div class="g-recaptcha" style="transform:scale(0.50);-webkit-transform:scale(0.85);transform-origin:0 0;-webkit-transform-origin:0 0;" data-sitekey="<?php echo $this->config->item('google_key') ?>">
			</div>
